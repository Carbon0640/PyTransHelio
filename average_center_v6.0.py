...
#==================控件生成========================================
    ...
    def _update_settings_visibility(self):
        ...
        if current_group in ["group1", "group2"]:
            self._clear_polygon()
            self._init_selector()
            self.output_label.config(text=f"Please start selecting the{'N' if current_group=='group1' else 'S'}-footpoint area", fg="blue")
        else:
...
#==================坐标计算=================
    def onselect(self, verts):  # 选区
        ...
        try:
            ...
            if hasattr(self, f'marked_point_{current_group}') and getattr(self, f'marked_point_{current_group}') is not None:
